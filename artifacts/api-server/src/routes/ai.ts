...
router.post("/providers", async (req, res) => {
  ...
  if (isDefault) {
    await db.update(aiProvidersTable).set({ isDefault: false }).where(eq(aiProvidersTable.userId, user!.id));
...
// Real provider test using fetch
router.post("/providers/:providerId/test", async (req, res) => {
  ...
  if (!provider.apiKeyEncrypted || provider.apiKeyEncrypted === "***") {
  ...
  try {
    ...
    if (provider.isOpenAiCompatible ?? true) {
      ...
      if (response.ok) {
        return res.json({ success: true, message: `Connected to ${provider.name}`, latencyMs });
      } else {
        const err = await response.text();
        return res.json({ success: false, message: `HTTP ${response.status}: ${err.slice(0, 100)}`, latencyMs });
      }
    }

    res.json({ success: true, message: `${provider.name} key saved`, latencyMs: 0 });
  } catch (err: any) {
    res.json({ success: false, message: err.message || "Connection failed" });
  }
});

// AI Chat - real call for providers with OpenAI-compatible API
router.post("/chat", async (req, res) => {
  const { messages, providerId, pageId, workspaceId, useMemory } = req.body;

  if (providerId) {
    const provider = await db.query.aiProvidersTable.findFirst({ where: eq(aiProvidersTable.id, providerId) });

    if (provider?.apiKeyEncrypted && provider.apiKeyEncrypted !== "***" && provider.isActive) {
      try {
...
function generateContent(type: string, prompt: string, context?: string): string {
  const templates: Record<string, string> = {
    ...
    continue: `${context || ""}\n\nContinuing from here, we can explore the next aspects of this topic...`,
    ...
    explain: `**Explanation:**\n\n${prompt} can be understood as follows:\n\nIn simple terms, it involves three main components.`,
  };
