...
router.patch("/:databaseId", async (req, res) => {
  const { name, defaultView, properties } = req.body;
...
router.post("/:databaseId/rows", async (req, res) => {
  ...
  const id = randomUUID();
  const count = await db.select().from(dbRowsTable).where(eq(dbRowsTable.databaseId, req.params.databaseId));
  await db.insert(dbRowsTable).values({ id, databaseId: req.params.databaseId, properties: properties || {}, order: count.length });
